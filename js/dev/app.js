...
function initSliders() {
  ...
  function initProjectsSlider() {
    if (window.innerWidth < 992 && !projectsSlider) {
      projectsSlider = new Swiper(".js-projects-slider", {
        modules: [Pagination],
        observer: true,
        observeParents: true,
        slidesPerView: 1,
    ...
    } else if (window.innerWidth >= 992 && projectsSlider) {
...
function handleScrollSection(sectionSelector, className, scrollPercentage) {
  const sections = document.querySelectorAll(sectionSelector);
  if (!sections.length) {
    console.warn(`Секція ${sectionSelector} не знайдена.`);
    return;
  }
  const viewportHeight = window.innerHeight;
  const offsetPx = Math.round(viewportHeight * (scrollPercentage / 100));
...
document.addEventListener("DOMContentLoaded", () => {
  handleScrollSection(".why-choose", "scroll-section", 80);
});
function headerScroll() {
  const header = document.querySelector("[data-fls-header-scroll]");
  const headerShow = header.hasAttribute("data-fls-header-scroll-show");
  header.dataset.flsScrollShow ? header.dataset.flsScrollShow : 500;
